...
public class ListBoxFrame extends ControlFrame implements ScrollBarFrame.ScrollBarChangeListener {
	...
	private final List<SingleStringFrame> maxPlayersValueFrames = new ArrayList<>();
	private final List<BackdropFrame> maxPlayersIconFrames = new ArrayList<>();
	...
	private void positionChildren(final GameUI gameUI, final Viewport viewport) {
		for (final BackdropFrame frame: this.maxPlayersIconFrames) {
			if (frame == null) continue;
			frame.positionBounds(gameUI, viewport);
		}
		for (final SingleStringFrame frame : this.maxPlayersValueFrames) {
			if (frame == null) continue;
			frame.positionBounds(gameUI, viewport);
		}
	...
	protected void internalRender(final SpriteBatch batch, final BitmapFont baseFont, final GlyphLayout glyphLayout) {
		...
		// 	frame.render(batch, baseFont, glyphLayout);
		// }
		for (int i = 0; i < this.stringFrames.size(); i++) {
			this.stringFrames.get(i).render(batch, baseFont, glyphLayout);
			if (this.maxPlayersIconFrames.get(i) != null)
				this.maxPlayersIconFrames.get(i).render(batch, baseFont, glyphLayout);
			if (this.maxPlayersValueFrames.get(i) != null)
				this.maxPlayersValueFrames.get(i).render(batch, baseFont, glyphLayout);
		}
		if (this.scrollBarFrame != null) {
	...
	private void updateUI(final GameUI gameUI, final Viewport viewport) {
		...
		final int numStringFrames = (int) Math.min(this.listItems.size(),
				(Math.floor((this.renderBounds.height - (this.listBoxBorder * 2)) / 30)));

		final StringFrame maxPlayerOriginal = (StringFrame) gameUI.getFrameByName("MaxPlayersValue", 0);
		...
		if (numStringFrames != this.stringFrames.size()) {
			...
			this.maxPlayersIconFrames.clear();
			this.maxPlayersValueFrames.clear();
			for (int stringFrameIndex = 0; stringFrameIndex < numStringFrames; stringFrameIndex++) {
				...
				SingleStringFrame maxPlayersValueFrame = new SingleStringFrame("MapValueY_" + index, this, Color.YELLOW, TextJustify.CENTER, TextJustify.MIDDLE, maxPlayerOriginal.getFrameFont());
				BackdropFrame maxPlayersIconFrame =  (BackdropFrame) gameUI.createFrameByType("BACKDROP", "MapIconY_" + index, this, "", 0);;
				if (index < this.listItems.size()) {
					...
					try {
						...
						War3MapW3i mapInfo = map.readMapInformation();
						WTS objDat = Warcraft3MapObjectData.loadWTS(map);
						gameUI.setMapStrings(objDat);
						stringFrame.setText(gameUI.getTrigStr(mapInfo.getName()));

						// maxPlayersValueFrame = (StringFrame) gameUI.getFrameByName("MaxPlayersValue", 0);
						maxPlayersValueFrame.setText(Integer.toString(mapInfo.getPlayers().size()));

						// maxPlayersIconFrame = (BackdropFrame) gameUI.getFrameByName("MaxPlayersIcon", 0);
						maxPlayersIconFrame.setWidth(30);
						maxPlayersIconFrame.setHeight(30);
						if (mapInfo.hasFlag(War3MapW3iFlags.MELEE_MAP)){
							maxPlayersIconFrame.setBackground(gameUI.loadTexture("ui\\widgets\\glues\\icon-file-melee.blp"));
						} else {
							maxPlayersIconFrame.setBackground(gameUI.loadTexture("ui\\widgets\\glues\\icon-file-ums.blp"));
						}
					}catch(Exception e){
						e.printStackTrace();
					}
				...
				stringFrame.setWidth(this.renderBounds.width - (this.listBoxBorder * 2) - 30);
				...
				if (prev != null) {
					...
					if (maxPlayersIconFrame == null) {
						stringFrame.addSetPoint(new SetPoint(FramePoint.TOPLEFT, prev, FramePoint.BOTTOMLEFT,0, 0));
						prev = stringFrame;
					} else {
						maxPlayersIconFrame.addSetPoint(new SetPoint(FramePoint.TOPLEFT, prev, FramePoint.BOTTOMLEFT,
							0, 0));
						maxPlayersValueFrame.addSetPoint(new SetPoint(FramePoint.CENTER, maxPlayersIconFrame, FramePoint.CENTER,
							0, 0));
						stringFrame.addSetPoint(new SetPoint(FramePoint.LEFT, maxPlayersIconFrame, FramePoint.RIGHT,
							0, 0));
						prev = maxPlayersIconFrame;
					}
				} else {
					...
					if (maxPlayersIconFrame == null) {
						...
						maxPlayersIconFrame.addSetPoint(new SetPoint(FramePoint.TOPLEFT, this, FramePoint.TOPLEFT,
							this.listBoxBorder, -this.listBoxBorder));
						maxPlayersValueFrame.addSetPoint(new SetPoint(FramePoint.CENTER, maxPlayersIconFrame, FramePoint.CENTER,
							0, 0));
						stringFrame.addSetPoint(new SetPoint(FramePoint.LEFT, maxPlayersIconFrame, FramePoint.RIGHT,
				...
				this.maxPlayersIconFrames.add(maxPlayersIconFrame);
				this.maxPlayersValueFrames.add(maxPlayersValueFrame);
				// prev = stringFrame;
				if (selected) {
					// this.selectionFrame
					...
					this.selectionFrame
							.addSetPoint(new SetPoint(FramePoint.TOPLEFT, maxPlayersIconFrame, FramePoint.TOPLEFT, 0, 0));
					this.selectionFrame.addSetPoint(
							new SetPoint(FramePoint.BOTTOMRIGHT, stringFrame, FramePoint.BOTTOMRIGHT, 0, 0));
					foundSelected = true;
				} else if (mousedOver) {
			...
			for (int stringFrameIndex = 0; stringFrameIndex < numStringFrames; stringFrameIndex++) {
				...
				SingleStringFrame maxPlayersValueFrame = this.maxPlayersValueFrames.get(stringFrameIndex);
				BackdropFrame maxPlayersIconFrame = this.maxPlayersIconFrames.get(stringFrameIndex);
				if (index < this.listItems.size()) {
					// stringFrame.setText(this.listItems.get(index));
					try {
						War3Map map = War3MapViewer.beginLoadingMap(gameUI.getDataSource(), this.listItems.get(index));
