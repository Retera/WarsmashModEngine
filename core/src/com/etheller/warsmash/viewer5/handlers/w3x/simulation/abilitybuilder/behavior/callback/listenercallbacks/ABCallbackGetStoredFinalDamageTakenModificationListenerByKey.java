...
public class ABCallbackGetStoredFinalDamageTakenModificationListenerByKey extends ABFinalDamageTakenModificationListenerCallback {
	...
	public ABFinalDamageTakenModificationListener callback(CSimulation game, CUnit caster, Map<String, Object> localStore, final int castId) {
		if (instanceValue == null || instanceValue.callback(game, caster, localStore, castId)) {
			return (ABFinalDamageTakenModificationListener) localStore.get(ABLocalStoreKeys.combineUserInstanceKey(key.callback(game, caster, localStore, castId), castId));
		} else {
			return (ABFinalDamageTakenModificationListener) localStore.get(ABLocalStoreKeys.combineUserKey(key.callback(game, caster, localStore, castId), castId));
		}
	}
