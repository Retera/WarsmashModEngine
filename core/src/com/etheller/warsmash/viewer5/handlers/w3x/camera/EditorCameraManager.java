...
public final class EditorCameraManager extends CameraManager {
	...
	private final int currentPreset = 0;
	private float fov;
	...
	public EditorCameraManager(final CameraPreset[] presets, final CameraRates cameraRates) {
		...
		this.userDistance = this.presetCameras[0].getDistance();
	}
	...
	private void updateCamera(final CameraSetup cameraPreset, final CameraRates cameraRate) {
		...
		this.distance = this.userDistance; // applyAtRate(this.distance, this.userDistance, cameraRate.distance);
	...
	public void scrolled(final int amount) {
		this.userDistance += amount * getCurrentRates().distance;
		clearCustomSetup();
	}
