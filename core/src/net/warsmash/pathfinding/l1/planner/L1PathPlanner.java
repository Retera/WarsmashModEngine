...
import net.warsmash.pathfinding.l1.Graph;
...
public class L1PathPlanner implements PathPlanner {
	...
	private L1PathPlanner(final Geometry geometry, final Graph graph, final INode root) {
	...
	public static void connectList(final List<Vertex> nodes, final Geometry geom, final Graph graph,
	...
	public static void connectNodes(final Geometry geom, final Graph graph, INode node, final boolean target,
		...
		while (node != null) {
			...
			final int idx = BSearch.search(buckets, y, L1PathPlanner::compareBucket);
			if (idx >= 0) {
	...
	public double search(final double tx, final double ty, final double sx, final double sy, final List<Point> outo) {
		...
		// Run A*
		final double dist = graph.search();
	...
	public static L1PathPlanner create(final PathfinderGrid grid) {
