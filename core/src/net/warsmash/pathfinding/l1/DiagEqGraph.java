...
public class DiagEqGraph extends AbstractGraph {
	...
	public static double heuristicDistance(final double ax, final double ay, final double bx, final double by) {
	...
	public void resetS(final Vertex v) {
		v.heuristic = heuristic(this.landmarkDist, this.dstX, this.dstY, v);
	}
	...
	public void addT(final Vertex v) {
		if ((v.state & 1) == 0) {
			...
			// Update heuristic
			final double d = heuristicDistance(v.x, v.y, this.dstX, this.dstY);
		...
		for (int k = 0; k < NUM_LANDMARKS; ++k) {
			...
			for (Vertex toVisit = v; toVisit != NIL;) {
				...
				for (int i = 0; i < adj.size(); i++) {
					...
					final double d = w + heuristicDistance(v.x, v.y, u.x, u.y);
					if (u.state == 0) {
						u.state = 1;
						u.weight = d;
	...
	// Runs a* on the graph
	public double search(final boolean backupMode) {
		...
		final boolean willFail = (this.lastS == null) || (this.lastT == null)
		...
		if (!willFail) {
			...
			for (Vertex toVisit = this.toVisit; toVisit != NIL;) {
				...
				for (int i = 0; i < n; ++i) {
					...
					final double vd = d + heuristicDistance(v.x, v.y, nx, ny);
					if (state < 2) {
