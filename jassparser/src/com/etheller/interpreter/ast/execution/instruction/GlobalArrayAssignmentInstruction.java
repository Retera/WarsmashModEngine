...
public class GlobalArrayAssignmentInstruction implements JassInstruction {
	private final int globalId;

	public GlobalArrayAssignmentInstruction(final int globalId) {
	...
	public void run(final JassThread thread) {
		...
		final ArrayJassValue arrayValue = globalValue.visit(ArrayJassValueVisitor.getInstance());
		if (arrayValue != null) {
			...
			if ((indexInt != null) && (indexInt >= 0)) {
				...
				throw new JassException(thread.globalScope, "Attempted to assign global " + this.globalId + "["
						+ indexInt + "], which was an illegal index", null);
			}
		}
		else {
