...
public class CAbilityLoad extends AbstractGenericSingleIconActiveAbility {
    ...
    public static CAbilityLoad getTransportLoad(final CSimulation game, final CUnit caster, final CUnit transport, final boolean ignoreRange, final boolean ignoreDisabled) {
        for (CAbility potentialLoadAbility : transport.getAbilities()) {
            if (potentialLoadAbility instanceof CAbilityLoad) {
                CAbilityLoad abilityLoad = (CAbilityLoad) potentialLoadAbility;
                BooleanAbilityActivationReceiver transportUnitReceiver = BooleanAbilityActivationReceiver.INSTANCE;
                abilityLoad.checkCanUse(game, transport, OrderIds.smart, transportUnitReceiver);
                // NOTE: disabled load ability should enable later in case of under construction entangled gold mine
                if (transportUnitReceiver.isOk() || (ignoreDisabled && abilityLoad.isDisabled())) {
                    ExternStringMsgTargetCheckReceiver<CWidget> transportUnitTargetCheckReceiver = ExternStringMsgTargetCheckReceiver.getInstance();
                    abilityLoad.checkCanTarget(game, transport, OrderIds.smart, caster, transportUnitTargetCheckReceiver.reset());
                    if (transportUnitTargetCheckReceiver.getTarget() != null || (ignoreRange && transportUnitTargetCheckReceiver.getExternStringKey() == CommandStringErrorKeys.TARGET_IS_OUTSIDE_RANGE)) {
                        return abilityLoad;
                    }
                }
            }
