...
public class CAbilityHero extends AbstractCAbility {
	...
	private final List<CAbility> learnedAbilities;
	private int xp;
	...
	public void onAdd(final CSimulation game, final CUnit unit) {
		if (this.heroLevel == 0) {
			...
			this.nameIndex = properNamesCount > 0 ? game.getSeededRandom().nextInt(properNamesCount) : 0;

			setHeroName(unitType);
		}
	}

	public void setHeroName(final CUnitType unitType) {
		String properName;
		final List<String> heroProperNames = unitType.getHeroProperNames();
		final int cnt = unitType.getProperNamesCount() > 0
				? Math.min(heroProperNames.size(), unitType.getProperNamesCount())
				: heroProperNames.size();
		if ((heroProperNames.size() > 0) && (cnt > 0)) {
			properName = heroProperNames.get(this.nameIndex % cnt);
		}
		else {
	...
	public void selectHeroSkill(final CSimulation game, final CUnit caster, final War3ID skillId) {
		...
		checkCanUse(game, caster, caster.getPlayerIndex(), skillId.getValue(), false, activationReceiver);
