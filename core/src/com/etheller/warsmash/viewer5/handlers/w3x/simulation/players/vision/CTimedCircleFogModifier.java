...
public class CTimedCircleFogModifier extends CFogModifier {
	...
	public CTimedCircleFogModifier(final CFogState fogState, final float radius, final float x, final float y,
	...
	public void onAdd(final CSimulation game, final CPlayer player) {
		this.endTurnTick = (int) Math
				.floor(game.getGameTurnTick() + (this.duration / WarsmashConstants.SIMULATION_STEP_TIME));
	}
	...
	public void update(final CSimulation game, final CPlayer player, final PathingGrid pathingGrid,
		...
		if (game.getGameTurnTick() > this.endTurnTick) {
			player.removeFogModifer(game, this);
			return;
		}
		final float radSq = this.radius * this.radius;
		fogOfWar.setVisible(pathingGrid.getFogOfWarIndexX(this.myX), pathingGrid.getFogOfWarIndexY(this.myY),
				this.state);
		...
		for (int y = 0; y <= (int) Math.floor(this.radius); y += 128) {
			for (int x = 0; x <= (int) Math.floor(this.radius); x += 128) {
				...
				if (distance <= radSq) {
					fogOfWar.setVisible(pathingGrid.getFogOfWarIndexX(this.myX - x),
							pathingGrid.getFogOfWarIndexY(this.myY - y), this.state);
					fogOfWar.setVisible(pathingGrid.getFogOfWarIndexX(this.myX - x),
							pathingGrid.getFogOfWarIndexY(this.myY + y), this.state);
					fogOfWar.setVisible(pathingGrid.getFogOfWarIndexX(this.myX + x),
							pathingGrid.getFogOfWarIndexY(this.myY - y), this.state);
