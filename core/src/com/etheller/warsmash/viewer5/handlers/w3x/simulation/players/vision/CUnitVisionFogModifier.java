...
public class CUnitVisionFogModifier extends CFogModifier {
	...
	private final boolean alwaysNightVision;
	...
	public void update(final CSimulation game, final CPlayer player, final PathingGrid pathingGrid,
		...
		if (this.enabled && !this.unit.isDead() && !this.unit.isHidden()) {
			final float sightRadius = game.isDay() && !this.alwaysNightVision
					? this.unit.getUnitType().getSightRadiusDay()
			...
			if (sightRadius > 0) {
				...
				final int myXi = game.getPathingGrid().getFogOfWarIndexX(myX);
				final int myYi = game.getPathingGrid().getFogOfWarIndexY(myY);
				final int maxXi = game.getPathingGrid().getFogOfWarIndexX(myX + sightRadius);
				final int maxYi = game.getPathingGrid().getFogOfWarIndexY(myY + sightRadius);
				for (int a = 1; a <= Math.max(maxYi - myYi, maxXi - myXi); a++) {
					if ((a * a) <= radSq) {
						fogOfWar.checkCardinalVision(game, pathingGrid, flying, myXi, myYi - a, myXi, (myYi - a) + 1,
								myX, myY - (a * CPlayerFogOfWar.GRID_STEP), myX,
