...
public class CAbilityTypeDefinitionAbilityTemplateBuilder
	...
	public CAbilityTypeDefinitionAbilityTemplateBuilder(final AbilityBuilderParser abilityBuilderParser) {
		super();
	...
	@Override
	protected CAbilityTypeAbilityBuilderLevelData createLevelData(final GameObject abilityEditorData, final int level) {
		...
		final List<War3ID> buffs = new ArrayList<>();
		final List<War3ID> effects = new ArrayList<>();
		for (final String buff : buffStrings) {
			if ((buff != null) && !buff.isEmpty()) {
				buffs.add(War3ID.fromString(buff));
			}
		}
		for (final String effect : effectStrings) {
			if ((effect != null) && !effect.isEmpty()) {
				effects.add(War3ID.fromString(effect));
			}
		}
		final String[] letters = WarsmashConstants.ABILITY_DATA_LETTERS;
		...
		if (checkDeps > 0) {
			...
			requirements = CUnitData.parseRequirements(requirementsString, requirementsLevelsString);
		}
	...
	protected CAbilityType<?> innerCreateAbilityType(final War3ID alias, final GameObject abilityEditorData,
		...
		if (abilityEditorData != null) {
			return new CAbilityTypeAbilityTemplateBuilder(alias, abilityEditorData.getFieldAsWar3ID(CODE, -1),
					abilityEditorData, levelData, this.parser);
		}
		else {
			return new CAbilityTypeAbilityTemplateBuilder(alias, alias, null, null, this.parser);
		}
	}
